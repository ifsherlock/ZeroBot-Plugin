...
var (
	engine = control.AutoRegister(&ctrl.Options[*zero.Ctx]{
		...
		Help: "自动解析 B站/抖音/TikTok/快手/微博/小红书/闲鱼/头条/小黑盒/Twitter 链接。\n" +
			...
			"- /媒体解析 b站画质 不限制|4K|1080P60|1080P+|1080P|720P|480P|360P\n" +
			"- /媒体解析 备用ytdlp 开启|关闭\n" +
			"- /媒体解析 清缓存",
...
func handleCommand(ctx *zero.Ctx) {
	...
	case "b站画质", "bilibili_quality":
		if len(args) < 2 {
			ctx.SendChain(message.Text("用法: /媒体解析 b站画质 不限制|4K|1080P60|1080P+|1080P|720P|480P|360P"))
			return
		}
		currentConf.BilibiliMaxQuality = args[1]
	case "resolution", "quality", "video_quality", "res":
		if len(args) < 2 {
